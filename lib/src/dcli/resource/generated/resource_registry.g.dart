// ignore: prefer_relative_imports
import 'package:dcli/dcli.dart';
import 'A00e8fb911bad2ed699bc39e0010fc2b5.g.dart';
import 'A01d04459549e6ee1a9bd5cc5d127a4b0.g.dart';
import 'A06bea26fce5648f0df5a9b7f59b6aa73.g.dart';
import 'A09ad6eb5733c0dc4e2f817d39c4a078f.g.dart';
import 'A1332bb16e5517f1b0503ad861aac56d4.g.dart';
import 'A16317090272f754c941e4b6f45dff149.g.dart';
import 'A18c77a77c3f0b6cdb02eb07f91221606.g.dart';
import 'A1eb845ab970ffe6208fb42af4a894494.g.dart';
import 'A2330f6f961c0bfb5630ec2d22cc10b98.g.dart';
import 'A297efcd5d45191fb70cccec6822e8d42.g.dart';
import 'A3242c7742263d9f15070d57b5455190d.g.dart';
import 'A3255464ce4b5134865eccd685e0bddce.g.dart';
import 'A33661602f23ced7aff98ee12e53f92f5.g.dart';
import 'A35b8a24e0c387e8aee1194f782e6f601.g.dart';
import 'A3ad2b55e3cb859e03393af3fe34b2443.g.dart';
import 'A4078a4d30e070b675dff996ae8efc76c.g.dart';
import 'A45870087402eb212a9984a39ff0517c2.g.dart';
import 'A4b127b725cdb260a7cd45d3a50491b12.g.dart';
import 'A4baa84d2b166ad5c7d95a8027f60197f.g.dart';
import 'A4c0ad69286b37acf908e1c3840a73f50.g.dart';
import 'A530bf66034a355ba82fc6b855b712aba.g.dart';
import 'A53458d0f7a6235a80ff0923fe2492222.g.dart';
import 'A5455ecf9deca828589566951e88042cf.g.dart';
import 'A568dcc061e0872b08fee0b0f5631d469.g.dart';
import 'A5692e6bea2ef8e2c647bbc7c4350e7e5.g.dart';
import 'A57ad1640d1e8bf72a9582e1c9df620db.g.dart';
import 'A5bd58c253cb49467e9fecc028b697b85.g.dart';
import 'A5d4abb4ce5aa55792aca02e7ee9a8d1a.g.dart';
import 'A5d651f24a14745d52a51d6fdbf925de6.g.dart';
import 'A5dc34467d083121d3f40b8bea61d136b.g.dart';
import 'A5eff10f65ed2c8359e80aaea15cb2991.g.dart';
import 'A613eab17e1396af07e9a2c6b0a1984df.g.dart';
import 'A62f796fa6d7edf5c14030978abf529da.g.dart';
import 'A66623fe501dfebe6f1a3e2fdc6c353a6.g.dart';
import 'A6aa1fefffa329899edd03defe1beda3b.g.dart';
import 'A7f85fab7862415acb472a23d07bc24c5.g.dart';
import 'A80b4a2ecd524ed81cb6b3f1b078b8afd.g.dart';
import 'A81ba6db9be37927758ad4d1b3b704f04.g.dart';
import 'A86b93c7843abd413a8372c00da339649.g.dart';
import 'A86cc730278e8a8687a6d774e3849895f.g.dart';
import 'A9af6f9ba0a1aaabc9a7f30ea1964ebbd.g.dart';
import 'Aa1da47c21e73962b7deb02a061ab4f75.g.dart';
import 'Aa2c25ace58385ff5d17e2964f198dd43.g.dart';
import 'Aa3f268d69ff3577d035df27e9b9b2800.g.dart';
import 'Aa7cb55c953965a98db3fff3a179dbeeb.g.dart';
import 'Aaca67f0dd722a9bd7d9954c1abf37836.g.dart';
import 'Aadc164b39e6c981591a1163d9c28ed9e.g.dart';
import 'Aaf4b838bbdfd2201d6088037df5d8424.g.dart';
import 'Aaf6432916c128cfb8eace96eda3e2c9a.g.dart';
import 'Aafca29a160703078b1f403535c51195e.g.dart';
import 'Ab192359fd11a5790b96943f26aed50a9.g.dart';
import 'Abd17acdf48fe12226ff30811929c6635.g.dart';
import 'Abd95856c717cbf9fa104554c181610aa.g.dart';
import 'Ac47d0d806ec79a051b171ad3050e8777.g.dart';
import 'Ac5d7050a167e1da76bdd7b92e982a75e.g.dart';
import 'Ac5ee828f9c68f5ed488859977eac9b76.g.dart';
import 'Ac94a9f6fcd609a049a966b8cf76f7531.g.dart';
import 'Ac9acdc8ecf89dda52925bfd5d7cdd0bc.g.dart';
import 'Acafe38d0c56f9ea4e818076d70b0d0c6.g.dart';
import 'Acb5197d6463711d85b53d4f6d32862b1.g.dart';
import 'Ace218cd80a3f6a56e94e276494bee4d6.g.dart';
import 'Adc50e211fafde9b14deb2ae3fb2502af.g.dart';
import 'Adf1749bffa593a5462b3029eb1212003.g.dart';
import 'Ae2f5746e54fa1d0d88858c833263983d.g.dart';
import 'Ae3f262b2908d5b2145ec991dca6c4d5b.g.dart';
import 'Ae4ba41d03adbb8256e4dca5135679a9c.g.dart';
import 'Ae4d67510782028d193aea2d45c6c0b1d.g.dart';
import 'Ae4d9e46b3f3f55845abb547d733b638f.g.dart';
import 'Aee2498eea986eb4d4b3268a38105fed2.g.dart';
import 'Aef6d7ced008982027439054801613335.g.dart';
import 'Af0aea8c0173fd222aea23b6cbcd408e8.g.dart';
import 'Af61e0032927093680a69524f89fe52ae.g.dart';
import 'Af7f238e133b5059d3e59191cb9a4383a.g.dart';
import 'Af93bd54deb10fc8c566b87e8632d5ff2.g.dart';
import 'Aff99ac9c08f93d659a1072f218718887.g.dart';

/// GENERATED -- GENERATED
///
/// DO NOT MODIFIY
///
/// This script is generated via [Resource.pack()].
///
/// GENERATED - GENERATED

class ResourceRegistry {
  /// Map of the packed files.
  /// Use the path of a packed file (relative to the resource directory)
  /// to access the packed resource and then call [PackedResource].unpack()
  /// to unpack the file.
  /// ```dart
  /// ResourceRegistry.resources['rules.yaml']
  ///     .unpack(join(HOME, '.mysettings', 'rules.yaml'));
  /// ```
  static const resources = <String, PackedResource>{
    'lib/presentation_layer/general/utils/utils.dart':
        A00e8fb911bad2ed699bc39e0010fc2b5(),
    'lib/main_prod.dart': A01d04459549e6ee1a9bd5cc5d127a4b0(),
    'lib/presentation_layer/stories/launch/launch_screen_view_model.dart':
        A06bea26fce5648f0df5a9b7f59b6aa73(),
    'assets/colors/neutrals.xml': A09ad6eb5733c0dc4e2f817d39c4a078f(),
    'lib/presentation_layer/stories/root/root_coordinator.dart':
        A1332bb16e5517f1b0503ad861aac56d4(),
    'lib/data_layer/repositories/auth_storage.dart':
        A16317090272f754c941e4b6f45dff149(),
    'lib/presentation_layer/general/abstract/alertable_mixin.dart':
        A18c77a77c3f0b6cdb02eb07f91221606(),
    'ios/Runner.xcodeproj/xcshareddata/xcschemes/dev.xcscheme':
        A1eb845ab970ffe6208fb42af4a894494(),
    'android/app/src/main/AndroidManifest.xml':
        A2330f6f961c0bfb5630ec2d22cc10b98(),
    'lib/presentation_layer/general/abstract/snack_bar_mixin.dart':
        A297efcd5d45191fb70cccec6822e8d42(),
    'lib/gen/colors.gen.dart': A3242c7742263d9f15070d57b5455190d(),
    '.prod.env': A3255464ce4b5134865eccd685e0bddce(),
    'lib/business_layer/network/api/user_api.dart':
        A33661602f23ced7aff98ee12e53f92f5(),
    'lib/localizations/app_en.arb': A35b8a24e0c387e8aee1194f782e6f601(),
    'lib/presentation_layer/general/enums/enum.dart':
        A3ad2b55e3cb859e03393af3fe34b2443(),
    'ios/Runner/GeneratedPluginRegistrant.m':
        A4078a4d30e070b675dff996ae8efc76c(),
    'ios/Flutter/Generated.xcconfig': A45870087402eb212a9984a39ff0517c2(),
    'lib/business_layer/extensions/build_context_helper.dart':
        A4b127b725cdb260a7cd45d3a50491b12(),
    'ios/Podfile': A4baa84d2b166ad5c7d95a8027f60197f(),
    'assets/images/google_logo.png': A4c0ad69286b37acf908e1c3840a73f50(),
    'lib/presentation_layer/constants/text_style_const.dart':
        A530bf66034a355ba82fc6b855b712aba(),
    'lib/main.dart': A53458d0f7a6235a80ff0923fe2492222(),
    'assets/colors/specials.xml': A5455ecf9deca828589566951e88042cf(),
    'ios/Runner/GeneratedPluginRegistrant.h':
        A568dcc061e0872b08fee0b0f5631d469(),
    'lib/business_layer/utils/flavor_config.dart':
        A5692e6bea2ef8e2c647bbc7c4350e7e5(),
    'lib/presentation_layer/general/extensions/theme_extension.dart':
        A57ad1640d1e8bf72a9582e1c9df620db(),
    'lib/data_layer/models/auth_token.dart':
        A5bd58c253cb49467e9fecc028b697b85(),
    'lib/data_layer/models/fresh.dart': A5d4abb4ce5aa55792aca02e7ee9a8d1a(),
    'ios/Flutter/flutter_export_environment.sh':
        A5d651f24a14745d52a51d6fdbf925de6(),
    'lib/env/dev_env.dart': A5dc34467d083121d3f40b8bea61d136b(),
    'lib/presentation_layer/general/base/base_imports.dart':
        A5eff10f65ed2c8359e80aaea15cb2991(),
    'ios/Runner/Info.plist': A613eab17e1396af07e9a2c6b0a1984df(),
    'lib/business_layer/network/error_exception.dart':
        A62f796fa6d7edf5c14030978abf529da(),
    'lib/business_layer/services/navigation_service.dart':
        A66623fe501dfebe6f1a3e2fdc6c353a6(),
    'lib/gen/assets.gen.dart': A6aa1fefffa329899edd03defe1beda3b(),
    'lib/presentation_layer/general/base/theme/app_colors_theme.dart':
        A7f85fab7862415acb472a23d07bc24c5(),
    'lib/business_layer/services/navigator_observer.dart':
        A80b4a2ecd524ed81cb6b3f1b078b8afd(),
    'assets/colors/alerts.xml': A81ba6db9be37927758ad4d1b3b704f04(),
    'lib/business_layer/services/root_routes.dart':
        A86b93c7843abd413a8372c00da339649(),
    'lib/presentation_layer/stories/launch/launch_screen_view_model_type.dart':
        A86cc730278e8a8687a6d774e3849895f(),
    'lib/business_layer/utils/locator.dart':
        A9af6f9ba0a1aaabc9a7f30ea1964ebbd(),
    'lib/presentation_layer/general/abstract/base_widget.dart':
        Aa1da47c21e73962b7deb02a061ab4f75(),
    'assets/colors/secondary.xml': Aa2c25ace58385ff5d17e2964f198dd43(),
    'ios/Runner.xcodeproj/xcshareddata/xcschemes/prod.xcscheme':
        Aa3f268d69ff3577d035df27e9b9b2800(),
    'android/local.properties': Aa7cb55c953965a98db3fff3a179dbeeb(),
    'android/app/build.gradle': Aaca67f0dd722a9bd7d9954c1abf37836(),
    'lib/localizations/app_localizations_en.dart':
        Aadc164b39e6c981591a1163d9c28ed9e(),
    'lib/business_layer/network/dio_client.dart':
        Aaf4b838bbdfd2201d6088037df5d8424(),
    'l10n.yaml': Aaf6432916c128cfb8eace96eda3e2c9a(),
    'lib/presentation_layer/general/base/undefined_screen.dart':
        Aafca29a160703078b1f403535c51195e(),
    'lib/localizations/app_localizations.dart':
        Ab192359fd11a5790b96943f26aed50a9(),
    'pubspec.yaml': Abd17acdf48fe12226ff30811929c6635(),
    'lib/env/prod_env.g.dart': Abd95856c717cbf9fa104554c181610aa(),
    'lib/presentation_layer/general/base/theme/app_text_theme.dart':
        Ac47d0d806ec79a051b171ad3050e8777(),
    'lib/main_dev.dart': Ac5d7050a167e1da76bdd7b92e982a75e(),
    '.vscode/launch.json': Ac5ee828f9c68f5ed488859977eac9b76(),
    'lib/presentation_layer/stories/launch/launch_screen.dart':
        Ac94a9f6fcd609a049a966b8cf76f7531(),
    'lib/gen/fonts.gen.dart': Ac9acdc8ecf89dda52925bfd5d7cdd0bc(),
    'assets/fonts/Montserrat-Regular.ttf': Acafe38d0c56f9ea4e818076d70b0d0c6(),
    'lib/data_layer/models/error_response.dart':
        Acb5197d6463711d85b53d4f6d32862b1(),
    'lib/business_layer/network/api/endpoints.dart':
        Ace218cd80a3f6a56e94e276494bee4d6(),
    'assets/colors/primary.xml': Adc50e211fafde9b14deb2ae3fb2502af(),
    'lib/env/dev_env.g.dart': Adf1749bffa593a5462b3029eb1212003(),
    'lib/presentation_layer/general/base/loading_view_model.dart':
        Ae2f5746e54fa1d0d88858c833263983d(),
    'lib/business_layer/network/abstract/app_api_type.dart':
        Ae3f262b2908d5b2145ec991dca6c4d5b(),
    'android/app/src/main/java/io/flutter/plugins/GeneratedPluginRegistrant.java':
        Ae4ba41d03adbb8256e4dca5135679a9c(),
    '.dev.env': Ae4d67510782028d193aea2d45c6c0b1d(),
    'lib/presentation_layer/app.dart': Ae4d9e46b3f3f55845abb547d733b638f(),
    'lib/business_layer/extensions/password_validate_extension.dart':
        Aee2498eea986eb4d4b3268a38105fed2(),
    'lib/presentation_layer/general/abstract/coordinator_type.dart':
        Aef6d7ced008982027439054801613335(),
    'lib/presentation_layer/stories/launch/launch_screen_view_model_route_type.dart':
        Af0aea8c0173fd222aea23b6cbcd408e8(),
    'lib/presentation_layer/general/base/actions.dart':
        Af61e0032927093680a69524f89fe52ae(),
    'lib/presentation_layer/general/base/app_theme.dart':
        Af7f238e133b5059d3e59191cb9a4383a(),
    'lib/env/prod_env.dart': Af93bd54deb10fc8c566b87e8632d5ff2(),
    'lib/presentation_layer/general/base/snack_notification.dart':
        Aff99ac9c08f93d659a1072f218718887(),
  };
}
